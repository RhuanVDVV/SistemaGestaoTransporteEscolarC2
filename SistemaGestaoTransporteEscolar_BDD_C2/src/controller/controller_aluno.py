...
class Controller_Aluno:
    ...
    def inserir_aluno(self) -> Aluno:
        ...
        data = dict(codigo=output_value, cpf = responsavel.get_CPF(), codigo_escola = int(escola.get_codigo_escola()), horario_aula = horario_aula, nome= nome, turma = turma, matricula = matricula)
    ...
    def excluir_aluno(self):
        ...
        if not self.verifica_existencia_aluno(oracle, codigo_aluno):
            ...
            escola = self.ctrl_motorista.valida_escola(oracle, df_aluno.codigo_escola.values[0])
           
            responsavel = self.valida_responsavel(oracle,df_aluno.cpf.values[0])
